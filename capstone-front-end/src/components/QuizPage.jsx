...
const QuizPage = () => {
  ...
  const handleQuizSelect = async (languageName, difficulty) => {
    ...
    switch (difficulty) {
      case 'BEGINNER': mappedDifficulty = 'EASY'; break;
      case 'INTERMEDIATE': mappedDifficulty = 'MEDIUM'; break;
      case 'ADVANCED': case 'EXPERT': mappedDifficulty = 'HARD'; break;
      default: console.error('Invalid difficulty level'); return;
    }
    try {
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setQuizResult({ ...resultData, correctAnswers: correct, incorrectAnswers: incorrect });
  ...
  return (
    <Container className="my-5">
      <h2 className="text-center mb-5">...... <span className="title-main">Choose a Quiz</span></h2>
      {!quizCompleted ? (
        !quizQuestions ? (
          languages.map((language) => (
            <Card key={language.programmingLanguageId} className="mb-4 auth-card text-white quiz-card">
              <Card.Body>
                    ...
                    skillLevels.map((level, index) => (
                      <Col key={level} xs={6} md={3}>
                        <Button
                          ...
                          disabled={index > skillLevels.indexOf(userLevels[language.name])}
                        >
                          {level}
                        </Button>
                      </Col>
